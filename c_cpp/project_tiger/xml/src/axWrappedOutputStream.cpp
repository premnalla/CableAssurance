...
axWrappedOutputStream::streamWriteInt(AX_INT32 v)
{
  ...
  AX_INT32 nWrite = write(getFd(), &v, sizeof(v));
  if (nWrite < 0) {
    axSocketFailInterface * failHdlr = getSocketFailHandler();
    if (failHdlr) {
      failHdlr->handleWriteFailure(getFd());
    }
  }
                                                          ...
                                                          AX_INT32 len)
{
  ...
  while(bytesToWrite > 0) {
    AX_INT32 nWrite = write(getFd(), bPtr, bytesToWrite);
    if (nWrite < 0) {
      axSocketFailInterface * failHdlr = getSocketFailHandler();
      if (failHdlr) {
        failHdlr->handleWriteFailure(getFd());
...
//********************************************************************
AX_INT32
axWrappedOutputStream::streamWrite(AX_INT8 b[], AX_INT32 length)
{
  return (streamWrite(b, 0, length));
}
