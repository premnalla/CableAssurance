...
//********************************************************************
void
myfping3::add_addr( char * name )
{
  HOST_ENTRY * p;
  struct addrinfo hints;
  struct addrinfo * result;
  struct addrinfo * result2;
  int numAddrsReturned;
  ...
  while (result) {
    numAddrsReturned++;
#if 0
    fprintf(stderr, "flags=%d; family=%d; socktype=%d; proto=%d; addr-len=%d; canonname=%s\n", 
            result->ai_flags,result->ai_family,result->ai_socktype,result->ai_protocol,
            result->ai_addrlen,result->ai_canonname);
#endif
    result = result->ai_next;
  }
  // fprintf(stderr, "num addrs returned = %d\n", numAddrsReturned);
  // exit (1);

  fprintf(stderr, "sizeof(sockaddr_storage)=%d\n", sizeof(sockaddr_storage));
  fprintf(stderr, "sizeof(in_addr)=%d\n", sizeof(in_addr));
  fprintf(stderr, "sizeof(in6_addr)=%d\n", sizeof(in6_addr));
  fprintf(stderr, "sizeof(sockaddr)=%d\n", sizeof(sockaddr));
  fprintf(stderr, "sizeof(sockaddr_in)=%d\n", sizeof(sockaddr_in));
  fprintf(stderr, "sizeof(sockaddr_in6)=%d\n", sizeof(sockaddr_in6));

  p->name = name;
  p->host = name;
  // p->saddr = *((sockaddr_storage *) result2->ai_addr);
  // p->saddr = *((sockaddr *) result2->ai_addr);
  p->saddr = *((sockaddr_in6 *) result2->ai_addr);
  p->addr_len = result2->ai_addrlen;
  p->timeout = timeout;
  p->running = 1;
...
myfping3::send_echo_request( HOST_ENTRY * h )
{
  ...
  sendRc = sendto( s, buffer, ping_pkt_size, 0,
    // ( struct sockaddr* )&h->saddr, sizeof( sockaddr_storage ) );
    ( struct sockaddr* )&h->saddr, h->addr_len );
...
myfping3::receive_echo_reply( HOST_ENTRY * h )
{
  ...
  if (memcmp(&response_addr, &h->saddr, slen)) {
    errno_crash_and_burn( "send/receive ports don't match" );
  }
