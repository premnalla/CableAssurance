...
TX_DEFINE_PARENT_TYPE(txHashTable,txObject)
...
int txHashTable::_index (const txObject* x, int find) const
{
	register txObject* p;
...
txObject* txHashTable::_insert (txObject* x, int index)
{
	if (index >= 0)
	{
		if (_fill*3 >= _size*2)
...
int txHashTable::resize (int minused)
{
	register int i;
	register txObject* p;
	register txObject** table;
	register int size, poly;
...
const txObject* txHashTable::insert (txObject* x)
{
	if (!_table) resize(64);
...
txObject* txHashTable::remove (const txObject* x)
{
...
void txHashTable::removeAndDestroy (const txObject* x)
{
...
void txHashTable::clearAndDestroy (void)
{
	if (!_table) return;
...
void txHashTable::stream (txOut& out) const
{
	txObject* node;
	...
	txHashTableIterator iter((txHashTable&) *this);
	...
	out.writeHeader(hdr, out.length() - (hdr + 4), TX_TABLE);
}

void txHashTable::destream (txIn& in)
{
...
txObject* txHashTableIterator::remove (void)
{
	register txObject* p = 0;
...
const txObject* txHashTableIterator::next (void)
{
	txObject* p;
...
const txObject* txHashTableIterator::key (void) const
{
	register txObject* p = 0;
