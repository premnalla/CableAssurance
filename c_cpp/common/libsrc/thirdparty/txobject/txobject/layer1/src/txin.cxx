...
int txIn::_variableLengthOverlay_ (txObject& object)
{
	...
	switch (type)
	{
		...
		case TX_OCTET_STRINGR0:
		{
			if (object.isClass(txOctetString::Type)) matched = 1;

			break;
		}
		case TX_OCTET_STRINGR1:
		{
			_currentLength_ -= 3;
		...
		case TX_TABLE:
		{
			if (object.isClass(txHashTable::Type)) matched = 1;
			break;
		}
		case TX_MAP:
		{
		...
		case TX_TREE:
		{
			if (object.isClass(txBTree::Type)) matched = 1;
			break;
		}
		case TX_GTYPE_INT32:
		{
			txUInt32 lu; lu.destream(*this); _cursor_ += 4;
...
unsigned long txIn::get (char*& o, unsigned long expected_type)
{
	...
	if (expected_type == TX_OCTET_STRING)
	{
		switch(type)
		{
			case TX_OCTET_STRINGR0:
