...
xsd__string SOAPStruct::getvarString()
{
	return varString ; 
}

void SOAPStruct::setvarString(xsd__string InValue)
{
	varString = InValue ; 
}

xsd__int* SOAPStruct::getvarInt()
{
	return varInt ; 
}

void SOAPStruct::setvarInt(xsd__int* InValue)
{
	varInt = InValue ; 
}

xsd__float* SOAPStruct::getvarFloat()
{
	return varFloat ; 
}

void SOAPStruct::setvarFloat(xsd__float* InValue)
{
	varFloat = InValue ; 
}
/*
...
int Axis_Serialize_SOAPStruct(SOAPStruct* param, IWrapperSoapSerializer* pSZ, bool bArray = false)
{
	...
	else
	{
		...
		const AxisChar* sPrefix = pSZ->getNamespacePrefix(Axis_URI_SOAPStruct, blnIsNewPrefix);
		if (!blnIsNewPrefix)
		{
			pSZ->serialize("<", Axis_TypeName_SOAPStruct, " xsi:type=\"", sPrefix, ":",
				Axis_TypeName_SOAPStruct, "\">", NULL);
		}
		else
		{
			pSZ->serialize("<", Axis_TypeName_SOAPStruct, " xsi:type=\"", sPrefix, ":",
...
void Axis_Delete_SOAPStruct(SOAPStruct* param, bool bArray = false, int nSize=0)
{
	if (bArray)
	{
		if (nSize > 0)
		{
			...
			{
				if ( (( SOAPStruct ** ) param)[count])
				{
					delete (( SOAPStruct ** ) param)[count];
					(( SOAPStruct ** ) param)[count] = NULL;
				}
			}
			delete [] ( SOAPStruct ** ) param;
		}
	}
	else
	{
