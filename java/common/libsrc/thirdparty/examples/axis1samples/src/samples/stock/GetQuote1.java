...
/**
 * This version of the ever so popular GetQuote shows how to use the Axis client
 * APIs with and without WSDL. The first flavor (getQuote1) will use WSDL to
 * prefill all of the data about the remote service. The second one (getQuote2)
 * will do it all manually. Either way the service is invoked it should produce
 * the exact same request XML and of course same results.
 * 
...
public class GetQuote1 {
	...
	/**
	 * This will use the WSDL to prefill all of the info needed to make the
	 * call. All that's left is filling in the args to invoke().
	 */
