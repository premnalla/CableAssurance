 ...
 *
 */
public class HfcCurrentStatus extends AbstractCurrentStatus {

	private static Log log = LogFactory.getLog(HfcCurrentStatus.class);

	public HfcCurrentStatus(Long resId) {
		super(resId);
		querySelf();
	}

	public HfcCurrentStatus(ProxyDbResultSet rs) {
		int i = 1;
		try {
			i++; // skip 'id' field
	...
	private void querySelf() {
		...
		try {
			 ...
			 * Create sql
			 */
			StringBuffer qryStr = new StringBuffer(GlobalQueries.QRY_CURRENT_HFC_STATUS);
