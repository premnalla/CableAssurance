 ...
 *
 */
public class CmPerfThreshSummary extends AbstractStatusSummary {

	private static Log log = LogFactory.getLog(CmPerfThreshSummary.class);
	 ...
	 * @param rs
	 */
	public CmPerfThreshSummary(ProxyDbResultSet rs) {
		super(rs);
	}
	 ...
	 * @param summaryOnly
	 */
	public CmPerfThreshSummary(ProxyDbResultSet rs, boolean summaryOnly) {
		super();
	...
	protected List queryDb(String qryStr, short batchSize, long startId) {
		...
		try {
			...
			while (rs.next()) {
				if (i < batchSize) {
					CmPerfThreshSummary s = new CmPerfThreshSummary(rs, true);
	 ...
	 * @return
	 */
	public List getLongestTca(Calendar fromDate, Calendar toDate,
			short batchSize, long startId) {
		List ret;

		StringBuffer qryStr = new StringBuffer(
				GlobalQueries.QRY_CM_TCA_SUMMARY);

		appendDateFilter(qryStr, fromDate, toDate);

		qryStr.append(GlobalQueries.QRY_CM_TCA_SUMM_GRPBY).append(
				GlobalQueries.QRY_CM_TCA_SUMM_ORDERBY_1);
	 ...
	 * @return
	 */
	public List getLongestTcaForCmts(long cmtsResId, Calendar fromDate,
			Calendar toDate, short batchSize, long startId) {
		List ret;

		StringBuffer qryStr = new StringBuffer(
				GlobalQueries.QRY_CM_TCA_SUMMARY_FOR_CMTS);
	 ...
	 * @return
	 */
	public List getLongestForHfc(long hfcResId, Calendar fromDate,
			Calendar toDate, short batchSize, long startId) {
		List ret;

		StringBuffer qryStr = new StringBuffer(
				GlobalQueries.QRY_CM_TCA_SUMMARY_FOR_HFC);
	 ...
	 * @return
	 */
	public List getHighestTcaFrequency(Calendar fromDate, Calendar toDate,
			short batchSize, long startId) {
		List ret;
		...
		qryStr.append(GlobalQueries.QRY_CM_TCA_SUMM_GRPBY).append(
				GlobalQueries.QRY_CM_TCA_SUMM_ORDERBY_2);
	 ...
	 * @return
	 */
	public List getHighestTcaFrequencyForCmts(long cmtsResId, Calendar fromDate,
			Calendar toDate, short batchSize, long startId) {
		List ret;
	 ...
	 * @return
	 */
	public List getHighestTcaFrequencyForHfc(long hfcResId, Calendar fromDate,
			Calendar toDate, short batchSize, long startId) {
		List ret;
