...
public class MarketAlarmImpl extends AbstractAlarmImpl implements AlarmEP {
	...
	public CurrentAlarmT[] getCurrentAlarms(InputTimeT fromTime,
		...
		// System.out.println("getCurrentAlarms(): ");
		...
		// System.out.println("fromSec=" + fromSec + "; toSec=" + toSec
		// + "; fromIndex=" + batch.getFromIndex() + "; toIndex="
		// + batch.getToIndex());
		...
		for (int i = 0; i < l.size(); i++) {
			...
			// System.out.println("\nGot alarm rom db:"
			// + svcAlm.getTypeDesc().toString());
			svcAlm.getAbstractAlarm().setTopologyKey(tK);
			al.add(svcAlm);
		}
		l.clear();
	...
	public CurrentAlarmT[] getCurrentAlarmsByType(String alarmType,
		...
		// System.out.println("type="+alarmType+"; subtype="+alarmSubType);
		...
		for (int i = 0; i < l.size(); i++) {
			...
			CurrentAlarmT svcAlm = new CurrentAlarmT(dbAlm);
			svcAlm.getAbstractAlarm().setTopologyKey(tK);
			// System.out.println("Type="+svcAlm.getAbstractAlarm().getAlarmType());
			al.add(svcAlm);
		}
		l.clear();
