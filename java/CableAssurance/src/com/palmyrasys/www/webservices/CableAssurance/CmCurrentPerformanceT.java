...
public class CmCurrentPerformanceT  implements java.io.Serializable {
	...
	private static Log log = LogFactory.getLog(CmCurrentPerformanceT.class);
     ...
     * @param dbPerf
     */
    public CmCurrentPerformanceT(CmCurrentPerformance dbPerf) {
		try {
	    	this.cmPerformance = new CmPerformanceT(dbPerf);
	    	this.recordedTime = dbPerf.getLastLogTime().toString();
		} catch (Exception e) {
			// e.printStackTrace();
			log.error(null, e);	
		}
