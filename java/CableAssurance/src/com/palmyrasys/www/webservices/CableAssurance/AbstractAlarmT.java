...
import com.palmyrasyscorp.db.tables.AbstractAlarmWithInfo;
...
public class AbstractAlarmT  implements java.io.Serializable {
	...
	private static Log log = LogFactory.getLog(AbstractAlarmT.class);
	 ...
	 * @param dbAlarm
	 */
	public AbstractAlarmT(AbstractAlarmWithInfo dbAlarm) {
		try {
			this.setAlarmId(dbAlarm.getAlarmId());
			this.setAlarmState(dbAlarm.getAlarmStateStr());
			this.setAlarmSubType(dbAlarm.getAlarmSubTypeStr());
			this.setAlarmTime(new BigInteger(dbAlarm.getAlarmDetectionTime()
					.toString()));
			this.setAlarmTimeUSec(new BigInteger(dbAlarm.getAlarmDetectionTimeUSec()
					.toString()));
			this.setAlarmType(dbAlarm.getAlarmTypeStr());
			this.setResourceId(new BigInteger(dbAlarm.getResourceId().toString()));
			dbAlarm.setAdditionalInfo();
			this.setResourceName(dbAlarm.getResourceName());
			this.setSoakDuration(dbAlarm.getSoakDuration().shortValue());
		} catch (Exception e) {
			// e.printStackTrace();
			log.error(null, e);	
		}
