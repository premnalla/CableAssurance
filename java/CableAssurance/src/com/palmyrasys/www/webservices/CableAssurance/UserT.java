...
import com.palmyrasyscorp.www.jsp.bean.NewUserFormBean;

public class UserT  implements java.io.Serializable {

	private static Log log = LogFactory.getLog(UserT.class);
     ...
     * @param dbUser
     */
    public UserT(User dbUser) {
    	try {
	        this.userId = dbUser.getAppUserId().toString();
	    	// System.out.println("set UserT(User) userId:");
	        this.firstName = dbUser.getFirstName();
	    	// System.out.println("set UserT(User) firstName:");
	        this.lastName = dbUser.getLastName();
	    	// System.out.println("set UserT(User) lastName:");
	        this.middleInitial = dbUser.getMiddleInitial();
	    	// System.out.println("set UserT(User) middleInitial:");
	        this.location = dbUser.getUserLocation();
	    	// System.out.println("set UserT(User) location:");
	        this.loginName = dbUser.getUserId();
	    	// System.out.println("set UserT(User) loginName:");
	        this.loginPassword = dbUser.getUserPw();
	    	// System.out.println("set UserT(User) loginPassword:");
	        if (dbUser.getIsUserActive().booleanValue() == true) {
	            this.isActive = new Short((short)1);        	
	        } else {
	            this.isActive = new Short((short)0);        	        	
	        }
	        this.roleName = dbUser.getRoleName();
	    	// System.out.println("set UserT(User) isActive:");
    	} catch (Exception e) {
			// e.printStackTrace();
			log.error(null, e);	    		
    	}
     ...
     * @param userBean
     */
    public UserT(NewUserFormBean userBean) {
    	try {
	        this.userId = "foo";
	        this.firstName = userBean.getFirstName();
	        this.lastName = userBean.getLastName();
	        this.middleInitial = userBean.getMiddleInitial();
	        this.location = userBean.getUserLocation();
	        this.loginName = userBean.getLoginUserId();
	        this.loginPassword = userBean.getLoginPw();
	        this.roleName = userBean.getRoleName();
	        
	    	short isActive;
	    	String beanIsUserDeactive = userBean.getAccountActive();
	    	
	    	// System.out.println("isActive="+beanIsUserDeactive);
	    	
	    	if (beanIsUserDeactive != null && beanIsUserDeactive.equals("on")) {
	    		// Deactivate has been checked by user...
	    		isActive = 0;
	    		// System.out.println("userBean.getAccountActive()="+userBean.getAccountActive());
	    	} else {
	    		isActive = 1;
	    		// System.out.println("userBean.getAccountActive()="+userBean.getAccountActive());
	    	}    	
	    	this.isActive = new Short(isActive);
    	} catch (Exception e) {
			// e.printStackTrace();
			log.error(null, e);	    		
    	}
