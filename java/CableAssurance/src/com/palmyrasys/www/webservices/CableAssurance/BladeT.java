...
public class BladeT  implements java.io.Serializable {
	...
	private static Log log = LogFactory.getLog(BladeT.class);
     ...
     * @param dbBlade
     */
    public BladeT(Blade dbBlade) {
		try {
			byte[] b = { 0 };
	        this.regionId = new BigInteger(b);
	        this.marketId = new BigInteger(b);
	        this.bladeId = new BigInteger(dbBlade.getId().toString());
	        this.name = dbBlade.getName();
	        this.host = dbBlade.getHost();    	
		} catch (Exception e) {
			// e.printStackTrace();
			log.error(null, e);	
		}
