...
public class AlarmHistoryT  implements java.io.Serializable {
	...
	//Prem:
	private static Log log = LogFactory.getLog(AlarmHistoryT.class);
     ...
     * @param dbAh
     */
    public AlarmHistoryT(AlarmHistory dbAh) {
    	try {
    		this.time = dbAh.getTimestamp().toString();
    		this.state = dbAh.getAlarmStateStr();
    		this.extraInfo = dbAh.getExtraInfo();
		} catch (Exception e) {
			// e.printStackTrace();
			log.error(null, e);
    	}
    ...
    public boolean isValid() {
    	...
    	if (this.time == null || this.state==null || this.extraInfo == null) {
    		ret = false;
    	} else {
    		ret = true;
    	}
