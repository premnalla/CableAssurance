...
import com.palmyrasyscorp.db.tables.ResourceTypes;

public class ResourceT  implements java.io.Serializable {
	
	//Prem:
	private static Log log = LogFactory.getLog(ResourceT.class);
	 ...
	 * @param dbRes
	 */
	public ResourceT(Resource dbRes) {
		try {
			this.resourceId = new BigInteger(dbRes.getResId().toString());
			switch (dbRes.getResType().shortValue()) {
			case ResourceTypes.RES_TYPE_CABLE_MODEM:
				this.resourceType = ResourceTypeT.CM;
				break;
			case ResourceTypes.RES_TYPE_CHANNEL:
				this.resourceType = ResourceTypeT.Channel;
				break;
			case ResourceTypes.RES_TYPE_CMS:
				this.resourceType = ResourceTypeT.CMS;
				break;
			case ResourceTypes.RES_TYPE_CMTS:
				this.resourceType = ResourceTypeT.CMTS;
				break;
			case ResourceTypes.RES_TYPE_EMTA:
				this.resourceType = ResourceTypeT.MTA;
				break;
			case ResourceTypes.RES_TYPE_HFC:
				this.resourceType = ResourceTypeT.HFC;
				break;
			default:
				log.warn("Unknown resource type: " + dbRes.getResType());
				break;
			}
		} catch (Exception e) {
			// e.printStackTrace();
